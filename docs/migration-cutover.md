...
Production cutover from the legacy Python/Bash control plane to
`xray-stackd` is complete. This document describes the resulting
production state and the rollback path.
...
- Legacy executable scripts were moved out of `/usr/local/bin` and archived under `/root/xray-audit-backups/legacy-files-removed-<timestamp>`.
