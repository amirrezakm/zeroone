...
internet) plus SFTP access to your target server. Total time: about
3 minutes once the bundle is downloaded.
...
Not sure which arch? On the target server run `uname -m`. `x86_64`
...
Your admins, users, and config are preserved.
...
The script makes a backup of `/etc/apt/sources.list` before
touching it, and restores the backup on any exit path (success
*or* failure) so your apt config is never left in an inconsistent
state.
...
- [`runflare-edge-deploy.md`](runflare-edge-deploy.md) — running a
