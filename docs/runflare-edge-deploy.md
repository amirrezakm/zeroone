...
WebSocket and XHTTP requests back to the origin server at
`185.128.139.68`.
...
```
client ── wss/https ──► your-app.runflare.run ── http ──► 185.128.139.68
...
| ----------------------- | --------------------------------------------------- |
| `/vless` (ws)           | `http://185.128.139.68:443/vless`                   |
| `/api/v1/events` (xhttp)| `http://185.128.139.68:80/api/v1/events` (nginx)    |
| anything else           | benign `OK` landing page                            |

---

## Origin side — already in place

Verified live on `185.128.139.68`:

- `xray` listens on `0.0.0.0:443` (VLESS-WS, plain — no TLS at origin).
- `xray` listens on `127.0.0.1:3002` (VLESS-XHTTP, plain).
- `nginx` listens on `0.0.0.0:80` and forwards `/api/v1/events` → `127.0.0.1:3002`.
- UFW already allows 80/tcp and 443/tcp from anywhere, so runflare's
  egress will reach both without firewall edits.

No origin changes are required.
...
| -------------------- | ----------------------------- | ---------------------------------------------------- |
| `ORIGIN_HOST`        | `185.128.139.68`              | Convenience: defaults WS_TARGET=:443, XHTTP_TARGET=:80 |
...
```sh
cd /Users/a.kimiyaei/Development/xray-stack-zeroone
runflare deploy
```
...
liara env set --app xray-edge-relay ORIGIN_HOST=185.128.139.68 WS_PATH=/vless XHTTP_PATH=/api/v1/events
...
runflare's egress. From a runflare-tailed log you'll see
`proxy error … dial tcp 185.128.139.68:443: connection refused`. Verify
from any external box: `curl -v http://185.128.139.68/api/v1/events`.
...
**Origin sees wrong `Host` header** — set `WS_HOST=185.128.139.68` and/or
`XHTTP_HOST=185.128.139.68` to match what Xray's inbound expects. The
