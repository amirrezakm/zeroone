...
1. On the server, edit `/usr/local/etc/zeroone/stack.json` (or whatever
   `zeroone` is loading) and update both `host` fields, then
   `enabled: true`.
2. `systemctl reload zeroone` (or `restart`) — or call
