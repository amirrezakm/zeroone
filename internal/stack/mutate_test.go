...
func TestUpsertClientEndpointDefaultsAndValidates(t *testing.T) {
	cfg := baseTestConfig()
	if err := cfg.UpsertClientEndpoint(ClientEndpoint{Name: "pars-pack", Host: "EDGE.PAYANEKHOSH.IR", TLS: true, Enabled: true}); err != nil {
		t.Fatal(err)
	}
	got := cfg.Server.ClientEndpoints[0]
	if got.Host != "edge.payanekhosh.ir" || got.Port != 443 || got.Network != "ws" || got.Path != "/vless" {
		t.Fatalf("unexpected endpoint defaults: %+v", got)
	}
	if err := cfg.UpsertClientEndpoint(ClientEndpoint{Name: "pars-pack", Host: "edge.payanekhosh.ir", Port: 443, Network: "tcp", Path: "/vless", TLS: true}); err == nil {
